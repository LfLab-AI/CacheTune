"""Spectral worker math and state regression tests (no vLLM/model weights needed)."""
...
MODULE_PATH = (Path(__file__).resolve().parents[1] / "vllm_blend" / "vllm"
               / "worker" / "cachetune_spectral.py")
SPEC = importlib.util.spec_from_file_location("spectral_worker_under_test", MODULE_PATH)
...
class SpectralTests(unittest.TestCase):
    def test_low_band_has_exact_floor_cutoff(self):
        ...
        torch.testing.assert_close(spectral.low_band_squared_norm(signal[:, None]), expected,
                                   rtol=1e-5, atol=1e-5)

    def test_tp_squared_statistics_reconstruct_full_head_norm(self):
        torch.manual_seed(18)
        raw_k, raw_v = torch.randn(17, 4, 8), torch.randn(17, 4, 8)
        full = 0.5 * (spectral.low_band_squared_norm(raw_k).sqrt()
                      + spectral.low_band_squared_norm(raw_v).sqrt())
        shards_k = sum(spectral.low_band_squared_norm(x) for x in raw_k.chunk(2, dim=1))
        shards_v = sum(spectral.low_band_squared_norm(x) for x in raw_v.chunk(2, dim=1))
    ...
    def test_chunk_boundaries_and_short_chunk(self):
        ...
        separate = torch.cat([spectral.low_band_squared_norm(first),
                              spectral.low_band_squared_norm(second)])
        combined = spectral.low_band_squared_norm(torch.cat([first, second]))
        self.assertFalse(torch.allclose(separate, combined))
        # T=1 gives F=1, floor(.5*F)=0, not an invented minimum of one bin.
        self.assertEqual(spectral.low_band_squared_norm(torch.ones(1, 2)).item(), 0)
...
class FakeWorker(spectral.CacheTuneSpectralWorkerMixin):
    def __init__(self):
        self.rank = 0
...
class WorkerStateTests(unittest.TestCase):
    ...
    def tearDown(self):
        self.worker.cachetune_spectral_release()

    def make_context(self, context_id="one", offset=0):
        self.worker.cachetune_spectral_begin(context_id)
        for i, layer in enumerate(self.worker._spectral_model().layers):
            k = torch.arange(28, dtype=torch.float32, device="cuda").reshape(7, 4)
            layer.self_attn.hack_kv = [k + offset + i, k * 2 + offset + i]
        stats = self.worker.cachetune_spectral_append_chunk(1, 7)
        self.assertEqual(tuple(stats["k_sq"].shape), (3, 6))
        self.assertEqual(stats["replication_factor"], [1, 1, 1])
        info = self.worker.cachetune_spectral_finalize(context_id, last_len=2)
        self.assertEqual(info["total_len"], 8)
        self.assertEqual(info["chunk_lengths"], [6])
        return self.worker._spectral_contexts[context_id]["full_kv"]

    def prepare(self, indices, context_id="one", **kwargs):
        return self.worker.cachetune_spectral_prepare(
    ...
    def test_suffix_and_immutable_snapshots_across_probes_and_contexts(self):
        ...
        meta = self.worker._spectral_model().cache_fuse_metadata
        self.assertTrue(meta["spectral_causal_mask"])
        ...
        self.assertFalse(meta["spectral_causal_mask"])
    ...
    def test_disk_sparse_payload_and_cpu_switch(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            meta = self.worker._spectral_model().cache_fuse_metadata
            path = meta["spectral_disk_paths"][0][0]
            torch.testing.assert_close(spectral._load_tensor(path), source[0][0][[1, 3, 4, 5]])
            self.assertTrue(meta["spectral_disk_enabled"])
            self.prepare([1, 3, 6, 7], storage="cpu")
            self.assertFalse(meta["spectral_disk_enabled"])
            self.worker.cachetune_spectral_release("one")
    ...
    def test_transfer_profile_reports_one_layer_without_suffix(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            for storage in ("cpu", "disk"):
                profile = self.worker.cachetune_spectral_profile_transfer(
