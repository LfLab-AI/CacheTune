...
HELPER = REPO / "vllm_blend/vllm/attention/spectral_attention.py"
BACKEND = REPO / "vllm_blend/vllm/attention/backends/xformers.py"
SPEC = importlib.util.spec_from_file_location("spectral_attention_helper", HELPER)
spectral_attention = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(spectral_attention)
make_spectral_causal_bias = spectral_attention.make_spectral_causal_bias
...
class SpectralAttentionTests(unittest.TestCase):
    def test_noncontiguous_queries_use_absolute_causality(self):
        ...
        bias = make_spectral_causal_bias(query, positions, 11)
    ...
    def test_mha_and_gqa_outputs_match_dense_selected_rows(self):
        ...
        for gqa in (False, True):
            with self.subTest(gqa=gqa):
                ...
                bias = make_spectral_causal_bias(selected_q[None], positions, n)
                if gqa:
        ...
        for dtype in (torch.float16, torch.bfloat16, torch.float32, torch.float64):
            for positions in (torch.arange(16), torch.arange(12, 16), torch.tensor([15])):
                with self.subTest(dtype=dtype, positions=positions.tolist()):
                    bias = make_spectral_causal_bias(torch.zeros(1, len(positions), 2, 8, dtype=dtype), positions, 16)
    ...
    def test_invalid_positions_and_query_shapes_fail(self):
        ...
        for positions in (torch.tensor([-1, 2]), torch.tensor([0, 11]),
            ...
            with self.assertRaises(ValueError):
                make_spectral_causal_bias(valid_query, positions, 11)
        for query in (torch.zeros(2, 2, 3, 8), torch.zeros(2, 3, 8),
                      valid_query.long(), torch.zeros(1, 0, 3, 8)):
            with self.assertRaises(ValueError):
                make_spectral_causal_bias(query, torch.tensor([0, 1]), 11)

    def test_backend_gate_preserves_legacy_and_reuses_spectral_bias(self):
        ...
        state['spectral_causal_mask'] = True
        with patch.dict(sys.modules, {'vllm.attention.spectral_attention': spectral_attention}):
            run(impl, query, key, key, metadata, 1, state)
            spectral_bias = recorded[-1]
            self.assertIsInstance(spectral_bias, torch.Tensor)
            self.assertTrue(torch.isneginf(spectral_bias[0, 0, 0, 1:]).all())
            run(impl, query, key, key, metadata, 2, state)
            self.assertIs(recorded[-1], spectral_bias)
            # A fresh status=1 rebuilds it for the next request's positions.
            state['imp_indices'] = torch.tensor([2, 4, 10])
            run(impl, query, key, key, metadata, 1, state)
            self.assertIsNot(recorded[-1], spectral_bias)
