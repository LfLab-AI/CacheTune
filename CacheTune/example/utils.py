...
def parse_generation(s):
    ...
    if s.startswith("Yes") or s.startswith("yes"):
        s = "Yes"
    elif tokens[0].startswith("No") or tokens[0].startswith("no"):
        s = "No"
    return s
...
def compute_f1(a_pred, a_gold, tokenizer):
    ...
    return f1
# def compute_f1(a_pred, a_gold, tokenizer=None):
#     a_pred = parse_generation(a_pred)
    
#     gold_text = normalize_answer(a_gold)
#     pred_text = normalize_answer(a_pred)
    
#     gold_toks = gold_text.split()
