...
def encode_like_wikimqa_reference(text, tokenizer):
...
parser = argparse.ArgumentParser(description="CacheTune WikiMQA Qwen inference script")
parser.add_argument("--model-path", type=str, default="/home/lifei/model/Qwen2.5-32B-Instruct")
...
for sample_idx, ex in enumerate(eval_dataset):
    ...
    doc_chunk_ids = [encode_like_wikimqa_reference(doc, tokenizer) for doc in doc_prompts]
    q_ids = encode_like_wikimqa_reference(q_prompt, tokenizer)
    ...
    s_start_full = encode_like_wikimqa_reference(prefix_prompt, tokenizer)
    ...
    # Hard reset between cached and baseline paths to avoid TP worker state leakage.
    run_on_all_workers(llm, "cachetune_reset_state")
