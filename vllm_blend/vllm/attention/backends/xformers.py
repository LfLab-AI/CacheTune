...
#         # TODO(lifei): The following `view`s can be saved
#         if status in [1, 2]:
#             key_old = old_kv[0].view(-1, self.num_kv_heads, self.head_size)
#             value_old = old_kv[1].view(-1, self.num_kv_heads, self.head_size)
...
#         # lifei: whether partial update or full update at check layer
#         if status in [1]:
#             imp_indices = cache_fuse_metadata["imp_indices"]
...
#             #if status in [2]:
#                 # TODO(lifei): This is full update/ Do we need partial update? 
#             #    PagedAttention.write_to_paged_cache(key, value, key_cache,
#             #                                        value_cache,
...
#         # TODO(lifei): need to change num prefill tokens accordingly
#         if status in [1,2]:
#             num_prefill_tokens = attn_metadata.num_prefill_tokens
#             num_decode_tokens = attn_metadata.num_decode_tokens
#             output = torch.empty_like(query)
...
#         if attn_metadata.attn_bias is None:
#             if self.alibi_slopes is None:
#                 # TODO(lifei): please pre-allocate mask for faster inference
...
#                                       device=device),
#                            diagonal=1)
#     #FIXME(lifei): The first 1 (bsz) is a hack
#     attn_mask = (attn_mask * torch.finfo(dtype).min).view(1, 
#                                                           1, 1, padded_len, padded_len) #FIXME(lifei): Now only focus on bsz=1
...
#     #FIXME(lifei): The first 1 (bsz) is a hack
#     attn_mask = (attn_mask * torch.finfo(dtype).min).view(#1,
#                                                           1, 1, seq_len, padded_len) #FIXME(lifei): Now only focus on bsz=1
#     attn_mask = attn_mask.expand(#1,
#                                  num_kv_heads, num_queries_per_kv,-1,-1)
